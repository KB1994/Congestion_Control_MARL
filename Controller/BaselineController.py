...
log_filename = f"./ryu_metrics_log_Baseline_{timestamp}.log"
...
# Baseline thresholds
# ----------------------------------------------------------
MAX_ACCEPTABLE_DELAY_MS = 100.0    # If RTT > 100 ms → reroute
MAX_ACCEPTABLE_LOSS     = 0.05     # If estimated loss > 5% → decrease_bw
MIN_FREE_BW_PERCENT     = 20.0     # If free_bw < 20% → decrease_bw
...
class BaselineController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Flow stats: { dpid: { flow_key: (pkt_count, byte_count, sec, nsec) } }
        self.flow_stats       = {}
        ...
        # Start baseline decision loop
        self.decision_interval = 5  # seconds between baseline decisions
        self.baseline_thread   = hub.spawn(self._baseline_decision_loop)

        logger.info("Baseline Ryu controller started. Logging initialized.")
    ...
    # ------------------------------------------------------------------------
    # Baseline decision loop (every 5 s)
    # ------------------------------------------------------------------------
    def _baseline_decision_loop(self):
        while True:
            for dpid, dp in self.datapaths.items():
                # 1) Estimate packet loss (mean across flows)
                loss_est = self._calculate_packet_loss()

                # 2) Get latest RTT (delay) in ms
                delay_ms = self.echo_latency.get(dpid, 0.0)

                # 3) Compute average free_bw% across all ports on this switch
                port_dict = self.port_stats.get(dpid, {})
                if port_dict:
                    free_list = []
                    for s in port_dict.values():
                        used_bw_pct = min((s['tx_bytes'] + s['rx_bytes']) * 8.0 / 1e6, 100.0)
                        free_list.append(100.0 - used_bw_pct)
                    avg_free_bw = float(np.mean(free_list))
                else:
                    avg_free_bw = 100.0

                # Decide baseline action
                if delay_ms > MAX_ACCEPTABLE_DELAY_MS:
                    action = 'reroute'
                elif loss_est > MAX_ACCEPTABLE_LOSS or avg_free_bw < MIN_FREE_BW_PERCENT:
                    action = 'decrease_bw'
                else:
                    action = 'hold'

                logger.info(
                    f"[Baseline Decision] DPID={dpid}  DELAY={delay_ms:.2f}ms  LOSS={loss_est:.2%}  "
                    f"FREE_BW≈{avg_free_bw:.2f}%  ACTION={action}"
                )

                # Apply the chosen action
                self._apply_action(dp, action)

            hub.sleep(self.decision_interval)
    ...
    def _apply_action(self, datapath, action):
        ...
        if action == 'hold':
        ...
        elif action == 'decrease_bw':
            # limit meter 1 to 10 Mbps (drop above 10 Mbps)
            ...
            logger.info(f"Applied 'decrease_bw' on DPID {datapath.id} (meter cap=10 Mbps)")
            return

        elif action == 'reroute':
            # raise meter to 50 Mbps, then force all flows to port=2
            ...
            logger.info(f"Applied 'reroute' on DPID {datapath.id} → forced output port=2")
            return

        # If action is unknown or 'hold', do nothing
        return

if __name__ == '__main__':
    app_manager.run_apps(['__main__.BaselineController'])
